...
size_t Text::GetClosestCharacter(Gwen::Point p)
{
	if(!m_Lines.empty())
	{
		...
		iChars -= pLine->Length();
		size_t iLinePos = pLine->GetClosestCharacter(Gwen::Point(p.x - pLine->X(), p.y - pLine->Y()));
		return iChars + iLinePos;
	}

	size_t iDistance = 4096;
	size_t iChar = 0;

	for(size_t i = 0; i < m_String.GetUnicode().length() + 1; i++)
	{
		Gwen::Rect cp = GetCharacterPosition(i);
		size_t iDist = abs(cp.x - p.x) + abs(cp.y - p.y);   // this isn't proper
...
{
	size_t iLine = GetLineFromChar(i);
